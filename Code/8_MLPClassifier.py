...
# Create pipeline
pipe_MLP_pca = make_pipeline(StandardScaler(), PCA(n_components=2), MLPClassifier(random_state=42))

# GridSearch & Cross-validation
# Setting parameters
param_grid = [{'mlpclassifier__hidden_layer_sizes': [10, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 2000, 5000, 10000],
               'mlpclassifier__activation': ['identity', 'logistic', 'tanh', 'relu'],
               'mlpclassifier__solver': ['lbfgs', 'sgd', 'adam'],
               'mlpclassifier__alpha': [0.0001, 0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000],
               'mlpclassifier__learning_rate': ['constant', 'invscaling', 'adaptive'],
               'mlpclassifier__max_iter': [10, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 2000, 5000, 10000]}]

# Set up grid search & cross validation for pca
gs_pca = GridSearchCV(estimator=pipe_MLP_pca, param_grid=param_grid, scoring='accuracy', cv=10, n_jobs=-1)
