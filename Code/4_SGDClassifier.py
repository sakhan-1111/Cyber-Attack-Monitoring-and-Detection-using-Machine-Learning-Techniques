...
# Create pipeline
pipe_SGD_pca = make_pipeline(StandardScaler(), PCA(n_components=2), SGDClassifier(random_state=42))
...
param_grid = [{'sgdclassifier__loss': ['hinge', 'log_loss', 'modified_huber', 'squared_hinge', 'perceptron', 'squared_error', 'huber', 'epsilon_insensitive', 'squared_epsilon_insensitive'],
               'sgdclassifier__alpha': [0.0001, 0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000],
               'sgdclassifier__max_iter': [10, 100, 1000, 10000, 100000]}]

# Set up grid search & cross validation for pca
gs_pca = GridSearchCV(estimator=pipe_SGD_pca, param_grid=param_grid, scoring='accuracy', cv=10, n_jobs=-1)
