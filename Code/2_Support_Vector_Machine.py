...
# Create pipeline
pipe_svm_pca = make_pipeline(StandardScaler(), PCA(n_components=2), SVC(random_state=42))

# GridSearch & Cross-validation
# Setting parameters
param_grid = [{'svc__C': [10000, 1000, 100, 10, 1, 0.1, 0.01, 0.001, 0.0001],
               'svc__kernel': ['linear', 'poly', 'rbf', 'sigmoid', 'precomputed'],
               'svc__cache_size': [100, 200, 300, 400, 500, 600, 700, 800, 900, 1000],
               'svc__max_iter': [-1, 1, 10, 100, 1000, 10000]}]

# Set up grid search & cross validation for pca
gs_pca = GridSearchCV(estimator=pipe_svm_pca, param_grid=param_grid, scoring='accuracy', cv=10, n_jobs=-1)
