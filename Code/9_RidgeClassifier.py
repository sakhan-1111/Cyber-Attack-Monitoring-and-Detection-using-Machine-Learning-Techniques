...
# Create pipeline
pipe_RG_pca = make_pipeline(StandardScaler(), PCA(n_components=2), RidgeClassifier(random_state=42))

# GridSearch & Cross-validation
# Setting parameters
param_grid = [{'ridgeclassifier__alpha': [0.0001, 0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000],
               'ridgeclassifier__max_iter': [10, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 2000, 5000, 10000],
               'ridgeclassifier__solver': ['svd', 'cholesky', 'lsqr', 'sparse_cg', 'sag', 'saga', 'lbfgs']}]

# Set up grid search & cross validation for pca
gs_pca = GridSearchCV(estimator=pipe_RG_pca, param_grid=param_grid, scoring='accuracy', cv=10, n_jobs=-1)
